...
export interface Brand {
  ...
  /**
   * Additive brand-level lifestyle image from Core (sourced by the Shopify
   * app's storefront scraping). Either a single URL or an ordered list —
   * tolerate both shapes and their absence.
   */
  backgroundImageUrl?: string | null;
  lifestyleImages?: (string | null)[] | null;
}

// First safe http(s) lifestyle URL for a brand, or null. Defensive about the
// wire shape: `lifestyleImages` may arrive as an array OR a bare string while
// Core's field settles, and either field may be absent/null.
export function brandLifestyleImageUrl(brand: Brand): string | null {
  const rawList = brand.lifestyleImages;
  const list = Array.isArray(rawList) ? rawList : typeof rawList === "string" ? [rawList] : [];
  const candidates = [brand.backgroundImageUrl, ...list];
  for (const url of candidates) {
    if (isSafeHttpUrl(url)) return url as string;
  }
  return null;
}
...
interface BrandHeroProps {
  /** Candidate image URLs in priority order: lifestyle → product → profile pic. */
  imageUrls: string[];
  alt: string;
...
// Static brand hero. Walks the candidate URLs in priority order (lifestyle
// image → product photo → IG profile picture), advancing on load failure, and
// lands on the deterministic gradient+initial when nothing loads.
function BrandHero({ imageUrls, alt, fallbackInitial, fallbackGradient, testKey }: BrandHeroProps) {
  const [failedCount, setFailedCount] = useState(0);
  // New candidate list → start again from the strongest candidate.
  const urlsSignature = imageUrls.join("\n");
  useEffect(() => {
    setFailedCount(0);
  }, [urlsSignature]);

  const src = imageUrls[failedCount] ?? null;
  if (!src) {
    return (
  ...
  return (
    <div className="absolute inset-0">
      <img
        ...
        data-testid={`img-brand-hero-${testKey}`}
      />
    </div>
  );
}
...
function BrandCard({ brand, onOpenBrand, igConnected, shopperDiscount }: BrandCardProps) {
  ...
  // When a lifestyle image is present every product appears in the carousel;
  // without one we promote the first imaged product into the hero so the card
  // never goes blank.
  const lifestyleImageUrl = brandLifestyleImageUrl(brand);
  const carouselProducts = lifestyleImageUrl
    ? products
  ...
  // Hero candidates in priority order: brand lifestyle image, first product
  // image, IG profile pic. The gradient+initial renders when all fail.
  const heroImageUrls = [
    lifestyleImageUrl,
    heroProduct?.image ?? null,
    brand.instagramProfilePictureUrl,
  ].filter((u): u is string => isSafeHttpUrl(u));
