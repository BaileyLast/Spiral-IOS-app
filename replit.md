...
Every story_mention received at `/webhooks/instagram-dm` is forwarded fire-and-forget to `POST https://spiral-merchant-dashboard.replit.app/api/instagram/story-mention` (header `x-spiral-internal-key`, 3s timeout, `[STORY-FORWARD]` prefix). Each messaging entry resolved to a Spiral customer is annotated inline with the shopper's real global `instagramUserId` so the dashboard can match without re-hitting the Graph API (Meta's `sender.id` is a per-app IGSID, not the global id). Unresolved entries ship without annotation; dashboard falls back to its own lookup. The forward payload also carries the matched merchant's `shopDomain` and `instagramBusinessAccountId` (top-level, alongside `messaging`) so the dashboard can identify the merchant without converting between Instagram's app-scoped id and the global webhook id; both are populated only after the story-mention merchant guard passes (so a Story tagging a different known account never names this merchant), and blank values are omitted. These fields are persisted on the retry-queue payload, so retried forwards include them too. DM verification-code messages are not forwarded.
...
- **Self-healing on rejection:** if a live Instagram call (`sendInstagramDM`, story-sender lookup) gets an auth error (OAuthException / code 190 / "session has expired"), `markJoinspiralTokenInvalid()` flags the stored token as expired and kicks an immediate reseed+refresh. This closes the "revoked-but-not-yet-expired" gap — a token can go bad before its expiry, and only an actual call reveals it. Recovery only fires on genuine auth errors (not transient/network blips), so a healthy refreshed token is never clobbered by a stale env value.
